...
async function loadProgram() {
  program.value = await offlineApi.getProgram(
...
async function saveEdit() {
  ...
  try {
    ...
    await offlineApi.updateProgram(
      userStore.currentUser!.id,
      program.value.id,
      {
...
async function loadExerciseHistory(di: number, ei: number) {
  ...
  try {
    exerciseHistory.value[key] = await offlineApi.getExerciseHistory(
      userStore.currentUser!.id,
      program.value.id,
      day.name.trim(),
      ex.name.trim(),
...
async function loadDayHistory(di: number) {
  ...
  try {
    dayHistory.value[di] = await offlineApi.getDayHistory(
      userStore.currentUser!.id,
      program.value.id,
      day.name.trim(),
    );
    ...
    <div v-if="editing">
      <div class="space-y-4 mb-6">
        <div
          ...
          :class="
            day.completed_at
              ? 'opacity-70 border-emerald-200 dark:border-emerald-900'
              : ''
          "
        >
          <!-- COMPLETED DAY -->
          <div v-if="day.completed_at">
            <div class="flex items-center gap-2 mb-2">
              <div
                class="w-6 h-6 rounded-full bg-emerald-100 dark:bg-emerald-900/50 flex items-center justify-center"
              >
                <svg
              ...
              <p class="flex-1 font-medium text-gray-600 dark:text-gray-400">
                {{ day.name }}
              </p>
            ...
            <p
              v-if="day.day_note"
              class="text-xs text-gray-500 dark:text-gray-400 mt-2 italic ml-8"
            >
              📝 {{ day.day_note }}
            </p>
          </div>
          ...
          <div v-else>
            ...
            <div
              ...
              class="mb-3 p-3 rounded-xl bg-blue-50 dark:bg-blue-950/50 border border-blue-200 dark:border-blue-900"
            >
              <p
                class="text-xs font-medium text-blue-700 dark:text-blue-300 mb-1.5"
              >
                📋 Коментарі до дня:
              </p>
              <div
                ...
                :key="hi"
                class="text-xs text-gray-600 dark:text-gray-400 mb-1 last:mb-0"
              >
            ...
            <div class="space-y-3 ml-2">
              ...
              >
                ...
                <div class="ml-6 space-y-1.5">
                  ...
                  >
                    <input
                      ...
                      type="number"
                      step="0.5"
                      class="w-16 px-2 py-1.5 border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-900 rounded-lg text-center text-sm focus:outline-none focus:border-blue-500"
                    />
                    ...
                    <input
                      ...
                      type="number"
                      min="1"
                      class="w-12 px-2 py-1.5 border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-900 rounded-lg text-center text-sm focus:outline-none focus:border-blue-500"
                    />
                  ...
                  <button
                    @click="addSet(di, ei)"
                    class="text-xs text-blue-500 hover:text-blue-700 dark:text-blue-400 font-medium py-1"
                  >
                    + підхід
                  </button>
                </div>
                ...
                <div
                  ...
                  class="ml-6 mt-2 p-3 rounded-xl bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-900"
                >
                  <p
                    class="text-xs font-medium text-amber-700 dark:text-amber-300 mb-1"
                  >
                    📝 З попередніх тренувань:
                  </p>
                  <div
                  ...
                  >
                    <span
                      ...
                      >{{ formatHistoryDate(h.completed_at) }}</span
                    >
              ...
              <button
                ...
                class="text-sm text-blue-500 hover:text-blue-700 dark:text-blue-400 font-medium py-1"
              >
                + вправа
              </button>
            </div>
    ...
    <div v-else>
      <div class="space-y-3">
        <div
          ...
          :class="{
            'border-l-status-completed': day.completed_at,
            'border-l-status-active':
              !day.completed_at && dayProgress(day).doneSets > 0,
        ...
        >
          ...
          >
                ...
                <div class="flex items-center gap-2 mt-1">
                  <div class="progress-bar flex-1 max-w-[100px]">
                    <div
                      ...
                      :class="
                        day.completed_at ? 'bg-emerald-500' : 'bg-blue-500'
                      "
                      :style="{
                        width:
                          (dayProgress(day).totalSets > 0
                            ? (dayProgress(day).doneSets /
                                dayProgress(day).totalSets) *
                              100
                            : 0) + '%',
                      }"
                    ></div>
            ...
            <button
              ...
              :class="
                day.completed_at ? 'btn-secondary btn-sm' : 'btn-success btn-sm'
              "
            >
          ...
          <div
            ...
            class="border-t border-gray-100 dark:border-gray-800 px-4 pb-4 pt-3 bg-gray-50/50 dark:bg-gray-800/30"
          >
            <div
              ...
              <div v-for="(ex, ei) in day.exercises" :key="ei" class="text-sm">
                ...
                <div v-if="ex.sets.length" class="ml-5 mt-1 space-y-0.5">
                  ...
                  >
                    ...
                    <svg
                      v-if="set.done"
                      class="w-3.5 h-3.5 text-emerald-500 ml-1"
                      fill="currentColor"
                      viewBox="0 0 20 20"
                    >
                      <path
      ...
      <div v-if="program.days.length === 0" class="text-center py-12">
        ...
        <p class="text-sm text-gray-400 dark:text-gray-500 mt-1">
          Натисніть "Редагувати" щоб додати дні
        </p>
      </div>
      ...
      <div class="mt-6 space-y-2">
        ...
        <button
          v-if="!isCompleted()"
          @click="finishProgram"
          class="btn-success w-full gap-2"
        >
